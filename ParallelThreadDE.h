 ...
 *
 * Slave:
 *	two queue: Remaining,Processing.
 *	When receive(finish) a task:
 *
 *	When request a task:
 *
 *  When all tasks is finished:
 *
 *
 *
 *
 */
class AutoTaskScheduler2:public TaskScheduler{
#define MESS_QUIT 0
#define MESS_ASSIGN 1
#define MESS_CANCEL 2
#define MESS_REFRESH 3
	...
	virtual const char *getName(){return "AutoTaskScheduler2";}
	void start(){
		//return only if all task is finished.
		if(mpi->isMaster()){
			int numTasks=getNumTasks();
